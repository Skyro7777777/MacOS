...
# Get the Tailscale IP + hostname for the config
TS_IP_FOR_CONFIG="$(cat "$STATE_DIR/tailscale-ip" 2>/dev/null || echo '0.0.0.0')"
TS_HOSTNAME="$(cat "$STATE_DIR/tailscale-hostname" 2>/dev/null || echo '')"

# --- 3a. get a proper TLS certificate from Tailscale (no browser warning!) ---
# Tailscale can provision a trusted HTTPS cert for <hostname>.<tailnet>.ts.net
# This eliminates the "Your connection is not private" warning.
# Requires: MagicDNS + HTTPS enabled in Tailscale admin console.
SUNSHINE_CERT=""
SUNSHINE_PKEY=""
if [ -n "$TS_HOSTNAME" ]; then
  # Get the full tailnet domain (Tailscale adds .ts.net or similar)
  TS_FQDN="$(sudo /opt/homebrew/bin/tailscale status --json 2>/dev/null | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('Self',{}).get('DNSName','').rstrip('.'))" 2>/dev/null || echo '')"
  if [ -n "$TS_FQDN" ]; then
    log "requesting Tailscale TLS cert for: $TS_FQDN"
    CERT_DIR="$SUNSHINE_CONFIG_DIR/certs"
    mkdir -p "$CERT_DIR"
    # tailscale cert writes <hostname>.crt + <hostname>.key
    if sudo /opt/homebrew/bin/tailscale cert --cert "$CERT_DIR/cert.pem" --key "$CERT_DIR/key.pem" "$TS_FQDN" 2>/dev/null; then
      # Make readable by the runner user
      sudo chown "$RUNNER_USER" "$CERT_DIR/cert.pem" "$CERT_DIR/key.pem" 2>/dev/null || true
      SUNSHINE_CERT="$CERT_DIR/cert.pem"
      SUNSHINE_PKEY="$CERT_DIR/key.pem"
      ok "Tailscale TLS cert obtained — no browser warning at https://$TS_FQDN:47990"
    else
      warn "tailscale cert failed — will use self-signed cert (browser warning expected)"
      log "  (enable HTTPS + MagicDNS in Tailscale admin console to fix this)"
    fi
  fi
fi

# Build the cert config lines (only if we have a real cert)
CERT_CONFIG=""
if [ -n "$SUNSHINE_CERT" ] && [ -n "$SUNSHINE_PKEY" ]; then
  CERT_CONFIG="pkey = $SUNSHINE_PKEY
cert = $SUNSHINE_CERT"
  # Also add the FQDN to CSRF origins
  CSRF_EXTRA=", https://$TS_FQDN:47990"
else
  CSRF_EXTRA=""
fi
...
csrf_allowed_origins = https://$TS_IP_FOR_CONFIG:47990, https://localhost:47990, https://127.0.0.1:47990$CSRF_EXTRA
$CERT_CONFIG
...
# Determine the web UI URL (use FQDN if we have a cert, IP otherwise)
if [ -n "$TS_FQDN" ] && [ -n "$SUNSHINE_CERT" ]; then
  WEB_UI_URL="https://$TS_FQDN:47990"
else
  WEB_UI_URL="https://$TS_IP_FOR_CONFIG:47990"
fi
...
log "    3. Open $WEB_UI_URL/pin in your browser "
